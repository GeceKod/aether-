...
def build_aether_movie_url(tmdb_id, title):
    """Film için Z-Stream sayfa adresini oluşturur."""
    slug = slugify(title)
    if slug:
        return f"{AETHER_BASE_URL}/tmdb-movie-{tmdb_id}-{slug}"
    return f"{AETHER_BASE_URL}/tmdb-movie-{tmdb_id}"

def build_aether_tv_url(tmdb_id, title):
    """Dizi için Z-Stream sayfa adresini oluşturur."""
    slug = slugify(title)
    if slug:
        return f"{AETHER_BASE_URL}/tmdb-tv-{tmdb_id}-{slug}"
    return f"{AETHER_BASE_URL}/tmdb-tv-{tmdb_id}"
